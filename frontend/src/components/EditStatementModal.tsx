...
const EditStatementModal: React.FC<EditStatementModalProps> = ({
  ...
  const isCleaningPassThroughItem = (item: any) => {
    const desc = (item?.description || '').toLowerCase();
    return Boolean(item?.isAutoGenerated) && desc.startsWith('cleaning -');
  };

  // Reservation status badge styling. Shown in the edit view only (not on the PDF)
  // so users can spot cancelled/modified bookings before finalizing a statement.
  const getStatusBadgeClasses = (status?: string) => {
    const s = (status || '').toLowerCase();
    if (s === 'cancelled' || s === 'canceled') return 'bg-red-100 text-red-800 border border-red-300 font-bold';
    if (s === 'modified') return 'bg-amber-100 text-amber-800 border border-amber-300';
    if (s === 'pending' || s === 'new' || s === 'inquiry' || s === 'awaiting_payment') return 'bg-yellow-100 text-yellow-800 border border-yellow-300';
    if (s === 'blocked') return 'bg-gray-200 text-gray-700 border border-gray-300';
    if (s === 'confirmed' || s === 'accepted') return 'bg-green-100 text-green-800 border border-green-300';
          ...
          {loading ? (
            ...
            <>
              ...
              <div>
                ...
                {/* Cleaning Pass-Through Section (read-only) */}
                {cleaningPassThroughFees.length > 0 && (
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <h4 className="text-sm font-semibold text-emerald-900">
                          Cleaning Pass-Through Fees ({cleaningPassThroughFees.length})
                        </h4>
                      </div>
                      <span className="text-xs text-emerald-700">Auto-generated from reservations</span>
                    </div>
                    <p className="text-xs text-emerald-700 mb-3">
                      Cleaning fee pass-through is enabled for this property. The guest-paid cleaning fee is
                      passed to the owner (net of PM commission). Real cleaning invoices are hidden from
                      expenses to avoid double-charging.
                    </p>
                    <div className="space-y-2">
                      {cleaningPassThroughFees.map((fee, index) => (
                        <div
                          key={`cleaning-passthrough-${index}`}
                          className="border rounded-lg p-3 bg-white border-emerald-200"
                        >
                          <div className="flex items-center justify-between">
                            <div>
                              <h4 className="font-medium text-gray-900">{fee.description}</h4>
                            ...
                            <div className="flex items-center text-emerald-700 font-semibold">
                              <DollarSign className="w-4 h-4 mr-1" />
                              {Math.abs(fee.amount).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </div>
                          </div>
                        </div>
                      ))}
                                ...
                                <div>
                                  ...
                                  <div className="text-sm text-gray-500">
                                    <div className="flex items-center space-x-4">
                                      ...
                                      {reservation.status && (
                                        <span
                                          className={`px-2 py-1 rounded-full text-xs uppercase tracking-wide ${getStatusBadgeClasses(reservation.status)}`}
                                          title="Shown in edit view only — not included on PDF"
                                        >
                                          {reservation.status}
                                        </span>
                                      )}
