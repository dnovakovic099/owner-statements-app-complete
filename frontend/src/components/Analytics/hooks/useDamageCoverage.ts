...
export interface DamageCoverageItem {
  propertyId: number;
  name: string;
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/analytics/damage-coverage?${queryParams.toString()}`,
        {
          method: 'GET',
          headers: {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to fetch damage coverage';
      setError(errorMessage);
      console.error('Error fetching damage coverage:', err);
    } finally {
      setLoading(false);
    }
