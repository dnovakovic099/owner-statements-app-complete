      ...
      <DialogContent className="sm:max-w-[1200px] w-[95vw] max-h-[85vh] p-0 gap-0 overflow-hidden" hideCloseButton>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="p-4 overflow-y-auto max-h-[calc(85vh-180px)]">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div className="space-y-4">
                ...
                {!generateAll ? (
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative" ref={ownerDropdownRef}>
                        <button
                          ...
                          className="flex h-9 w-full items-center justify-between rounded-md border border-gray-300 bg-white px-3 py-2 text-sm ring-offset-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                        >
                              ...
                              {filteredOwners.length === 0 ? (
                                ...
                                filteredOwners.map((owner) => (
                                  <div
                                    ...
                                    className={`px-3 py-1.5 cursor-pointer hover:bg-blue-50 flex items-center justify-between ${
                      ...
                      {selectedTags.length > 0 && (
                        <div className="bg-purple-50 border border-purple-200 rounded-md p-2 mt-2">
                          <div className="flex items-start space-x-2">
                            ...
                            />
                            <div>
                              <Label htmlFor="generateCombinedTag" className="cursor-pointer text-xs font-medium text-purple-900">
                                Combined statement per tag
                              </Label>
                              <p className="text-xs text-purple-700 mt-0.5">
                                {selectedTags.length === 1
                                  ? (generateCombined
                                      ? `ONE statement for all "${selectedTags[0]}" listings`
                                      : `SEPARATE statements per listing`)
                                  : (generateCombined
                                      ? `${selectedTags.length} combined statements`
                                      : `Separate per listing`)}
                              </p>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="bg-gray-50 border rounded-md p-3">
                        ...
                        <div className="text-xs text-gray-600 space-y-1">
                          <div className="flex justify-between">
                            <span>Owner:</span>
                            <span className="font-medium truncate ml-2">{owners.find(o => o.id.toString() === ownerId)?.name}</span>
                          </div>
                          <div className="flex justify-between">
                          ...
                          {selectedTags.length > 0 && (
                            <div className="flex justify-between">
                              <span>Tags:</span>
                              <span className="font-medium text-purple-600 truncate ml-2">{selectedTags.join(', ')}</span>
                            </div>
                          )}
                          <div className="flex justify-between">
                            <span>Period:</span>
                          ...
                          {(selectedPropertyIds.length > 1 || selectedTags.length > 0) && (
                            <div className="flex justify-between">
                              ...
                              <span className={`font-medium ${generateCombined ? 'text-green-600' : 'text-purple-600'}`}>
                                {selectedTags.length > 1
                                  ? (generateCombined ? `${selectedTags.length} Combined` : `Separate`)
                                  : selectedTags.length === 1
                                    ? (generateCombined ? '1 Combined' : `Separate`)
                                    : (generateCombined ? '1 Combined' : `${selectedPropertyIds.length} Separate`)}
                              </span>
                            </div>
                          )}
                        </div>
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-md p-3 text-center">
                    <div className="text-blue-600 font-medium text-sm mb-1">Bulk Generation Mode</div>
              ...
              <div className="space-y-3">
                ...
                {!generateAll && ownerId && filteredProperties.length > 0 && (
                  <>
                    <div className="relative">
                      <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4" />
                      <Input
                ...
                {!generateAll && ownerId ? (
                  ...
                  <div className="border rounded-md px-3 py-8 text-sm text-gray-500 bg-gray-50 text-center">
                    Select an owner first to see their properties
                  </div>
                ) : (
                  <div className="border rounded-md px-3 py-8 text-sm text-gray-500 bg-blue-50 text-center">
                    All properties will be included in bulk generation
                  </div>
                )}

                {selectedPropertyIds.length === 0 && ownerId && !generateAll && (
                ...
                {selectedPropertyIds.length > 1 && (
                  <div className="bg-green-50 border border-green-200 rounded-md p-2">
                    <div className="flex items-start space-x-2">
                      ...
                      />
                      <div>
                        <Label htmlFor="generateCombined" className="cursor-pointer text-xs font-medium text-green-900">
                          Generate combined statement
                        </Label>
                        <p className="text-xs text-green-700">
                          {generateCombined
                            ? `ONE statement for ${selectedPropertyIds.length} properties`
              ...
              {/* COLUMN 3 - Date Range & Calculation */}
              <div className="space-y-4">
                <div className="flex items-center space-x-2 text-gray-700 font-medium border-b pb-2">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        required
                        className="w-full h-9 px-2 text-sm border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">
                ...
                {/* Calculation Method */}
                <div className="space-y-2">
                  <Label className="text-sm">Calculation Method <span className="text-red-500">*</span></Label>
                    ...
                    <label className={`flex items-start p-2.5 border rounded-md cursor-pointer transition-all ${
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-500">
                          Include reservations that check out during period
                        </div>
                      </div>
                    </label>
                    <label className={`flex items-start p-2.5 border rounded-md cursor-pointer transition-all ${
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-500">
                          Prorate reservations by days in period
                        </div>
                      </div>
                    </label>
                  </div>
          ...
          <DialogFooter className="px-6 py-3 bg-gray-50 border-t">
            <Button type="button" variant="outline" onClick={onClose} disabled={isGenerating} size="sm">
              Cancel
            </Button>
