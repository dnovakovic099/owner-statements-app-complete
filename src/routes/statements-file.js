...
function computePassThroughCleaningFee(statement, reservation) {
    const guestPaid = reservation.cleaningFee || 0;
...
/**
 * Build a set of expense signatures from prior statements for cross-statement duplicate detection.
 * Returns a Map of signature -> { statementId, period } for lookup.
 */
function buildPriorExpenseSignatures(priorStatements) {
    ...
    for (const stmt of priorStatements) {
        ...
        for (const exp of expenses) {
            // Primary key: source + sourceId (for SecureStay expenses with unique IDs)
            ...
            // Fallback key: amount + date + description (for uploaded expenses)
...
/**
 * Check if an expense matches a prior statement signature.
 * Returns the match info or null.
 */
function matchExpenseToPrior(exp, signatureMap) {
    // Try source + sourceId first
    if (exp.source && exp.sourceId) {
        const key = `${exp.source}:${exp.sourceId}`;
        if (signatureMap.has(key)) return signatureMap.get(key);
    }
    // Fallback: amount + date + description
    const amount = Math.round(Math.abs(parseFloat(exp.amount) || 0) * 100);
    const desc = (exp.description || '').trim().toLowerCase();
    const date = exp.date || '';
    const fallbackKey = `${amount}|${date}|${desc}`;
    ...
    try {
        ...
        const formattedStatements = paginatedStatements.map(s => {
            ...
            if (s.reservations && s.reservations.length > 0) {
                ...
                const hasExcludeCleaning = statementPropertyIds.some(propId => {
                    const listing = listingMap.get(parseInt(propId));
                    return listing && listing.excludeCleaningFromCommission;
                });
                ...
                if (hasPassThrough || hasWaiver || hasExcludeCleaning) {
                    ...
                    for (const res of s.reservations) {
                        ...
                        const pmPercentage = listing ? StatementCalculationService.getEffectivePmFee(listing, res.createdAt) : (s.pmPercentage ?? 15);
                        const cleaningFeePassThrough = listing?.cleaningFeePassThrough || false;
                        const isCohostOnAirbnb = listing?.isCohostOnAirbnb || false;
                        const cohostCommissionCollected = listing?.cohostCommissionCollected || false;
                        ...
                        const luxuryFee = (res.isCustom && res.luxuryLodgingFee !== undefined)
                            ...
                            : commissionBase * (pmPercentage / 100);
                        ...
                        let grossPayout;
                        if (isCohostAirbnb) {
                            grossPayout = -StatementCalculationService.getCohostCommissionDeduction(cohostCommissionCollected, luxuryFeeToDeduct) - cleaningFeeForPassThrough;
        ...
        if (statement.cleaningFeePassThrough && statement.reservations && statement.reservations.length > 0) {
            ...
            if (!hasCleaningExpenses || !hasCleaningItems) {
                ...
                for (const res of statement.reservations) {
                    ...
                    const calculatedCleaningFee = computePassThroughCleaningFee(statement, res);
                    if (calculatedCleaningFee > 0 && checkoutInPeriod) {
...
async function generateCombinedStatement(req, res, propertyIds, ownerId, startDate, endDate, calculationType, group = null) {
    try {
        ...
        // Fetch reservations, expenses, and prior statement data in parallel
        ...
        for (const res of periodReservations) {
            ...
            const resPmPercentage = StatementCalculationService.getEffectivePmFee(resListingInfo, res.createdAt);
            ...
            let grossPayout;
            if (isCohostAirbnb) {
                grossPayout = -StatementCalculationService.getCohostCommissionDeduction(resCohostCommissionCollected, luxuryFeeToDeduct) - cleaningFeeForPassThrough;
            } else if (shouldAddTax) {
                grossPayout = clientRevenue - luxuryFeeToDeduct + taxResponsibility - cleaningFeeForPassThrough;
            } else {
                grossPayout = clientRevenue - luxuryFeeToDeduct - cleaningFeeForPassThrough;
        ...
        for (const exp of filteredExpenses) {
            const match = matchExpenseToPrior(exp, priorSignatures);
            if (match) {
                priorStatementFlaggedExpenses.push({ expense: exp, match });
                priorStatementDuplicateWarnings.push({
                ...
                nonDuplicateFilteredExpenses.push(exp);
            }
        }

        // Also check LL Cover expenses for prior statement duplicates
        const priorStatementFlaggedLLCover = [];
        const nonDuplicateLLCover = [];
        for (const exp of llCoverExpenses) {
            const match = matchExpenseToPrior(exp, priorSignatures);
            if (match) {
                priorStatementFlaggedLLCover.push({ expense: exp, match });
                priorStatementDuplicateWarnings.push({
                    type: 'prior_statement',
                    description: exp.description,
                    amount: exp.amount,
                ...
                nonDuplicateLLCover.push(exp);
            }
        }

        if (priorStatementDuplicateWarnings.length > 0) {
            logger.info(`Found ${priorStatementDuplicateWarnings.length} cross-statement duplicate expenses`, { context: 'StatementsFile', action: 'generateCombinedStatement' });
        }

        // Recalculate totals excluding prior-statement duplicates
        const adjustedTotalExpenses = nonDuplicateFilteredExpenses.reduce((sum, exp) => {
            const isUpsell = exp.amount > 0 || (exp.type && exp.type.toLowerCase() === 'upsell') || (exp.category && exp.category.toLowerCase() === 'upsell');
            return isUpsell ? sum : sum + Math.abs(exp.amount);
        }, 0);
        const adjustedTotalUpsells = nonDuplicateFilteredExpenses.reduce((sum, exp) => {
            const isUpsell = exp.amount > 0 || (exp.type && exp.type.toLowerCase() === 'upsell') || (exp.category && exp.category.toLowerCase() === 'upsell');
            return isUpsell ? sum + exp.amount : sum;
        }, 0);
        const adjustedOwnerPayout = grossPayoutSum + adjustedTotalUpsells - adjustedTotalExpenses;
        ...
        const statement = {
            ...
            totalExpenses: Math.round(adjustedTotalExpenses * 100) / 100,
            ...
            excludeCleaningFromCommission: parsedPropertyIds.some(propId => listingInfoMap[propId]?.excludeCleaningFromCommission),
            ...
            items: [
                ...
                // Prior statement duplicate expenses - hidden by default
                ...priorStatementFlaggedExpenses.map(({ expense: exp, match }) => {
                    const isUpsell = exp.amount > 0 || (exp.type && exp.type.toLowerCase() === 'upsell') || (exp.category && exp.category.toLowerCase() === 'upsell') || (exp.expenseType === 'extras');
                ...
                // LL Cover expenses are stored as hidden items for review (excluding prior statement duplicates)
                ...nonDuplicateLLCover.map(exp => {
                    const isUpsell = exp.amount > 0 || (exp.type && exp.type.toLowerCase() === 'upsell') || (exp.category && exp.category.toLowerCase() === 'upsell') || (exp.expenseType === 'extras');
                    const listing = exp.propertyId ? targetListings.find(l => parseInt(l.id) === parseInt(exp.propertyId)) : null;
                    const propertyLabel = listing ? (listing.nickname || listing.displayName || listing.name) : (exp.listing || 'General');
                ...
                // LL Cover expenses that are also prior statement duplicates
                ...priorStatementFlaggedLLCover.map(({ expense: exp, match }) => {
                    const isUpsell = exp.amount > 0 || (exp.type && exp.type.toLowerCase() === 'upsell') || (exp.category && exp.category.toLowerCase() === 'upsell') || (exp.expenseType === 'extras');
                    const listing = exp.propertyId ? targetListings.find(l => parseInt(l.id) === parseInt(exp.propertyId)) : null;
                    const propertyLabel = listing ? (listing.nickname || listing.displayName || listing.name) : (exp.listing || 'General');
...
router.post('/generate', async (req, res) => {
    ...
    try {
        ...
        const fetchPromises = [
            ...
            // Fetch prior statement expenses in parallel (for propertyId, use it; otherwise resolved later)
        ...
        // Pre-fetch all listing info needed for PM commission and payout calculation
        // For multi-property statements, batch-fetch all unique property IDs in one query
        let listingInfoMap = {};
        if (propertyId) {
            listingInfoMap[parseInt(propertyId)] = listingInfo;
        } else {
            const uniquePropIds = [...new Set(periodReservations.map(r => parseInt(r.propertyId)))];
            if (uniquePropIds.length > 0) {
                const dbListings = await ListingService.getListingsWithPmFees(uniquePropIds);
                dbListings.forEach(l => { listingInfoMap[l.id] = l; });
            }
        }
        ...
        if (propertyId) {
            // Single property - calculate PM commission per-reservation (accounts for PM fee transitions)
            for (const res of periodReservations) {
                const isAirbnb = res.source && res.source.toLowerCase().includes('airbnb');
                if (isAirbnb && (listingInfo?.isCohostOnAirbnb || false)) continue;
                const resPmFee = listingInfo ? StatementCalculationService.getEffectivePmFee(listingInfo, res.createdAt) : 15;
                const resRevenue = res.hasDetailedFinance ? res.clientRevenue : (res.grossAmount || 0);
                pmCommission += resRevenue * (resPmFee / 100);
            }
            pmPercentage = totalRevenue > 0 ? (pmCommission / totalRevenue) * 100 : (listingInfo?.pmFeePercentage ?? 15);
        } else {
            // Multi-property statement - calculate PM commission per reservation (listings pre-fetched above)
            for (const res of periodReservations) {
                const resListing = listingInfoMap[parseInt(res.propertyId)] || null;

                // Check if this is Airbnb and co-hosted
                const isAirbnb = res.source && res.source.toLowerCase().includes('airbnb');
                const isCohostForProperty = resListing?.isCohostOnAirbnb || false;
                ...
                const resPmFee = resListing ? StatementCalculationService.getEffectivePmFee(resListing, res.createdAt) : 15;
                // Use clientRevenue (prorated) for calendar-based statements
                const resRevenue = res.hasDetailedFinance ? res.clientRevenue : (res.grossAmount || 0);
        ...
        let grossPayoutSum = 0;
        for (const res of periodReservations) {
            const resListingInfo = propertyId ? listingInfo : (listingInfoMap[parseInt(res.propertyId)] || null);
            const resPmPercentage = resListingInfo ? StatementCalculationService.getEffectivePmFee(resListingInfo, res.createdAt) : 15;
            const resDisregardTax = resListingInfo?.disregardTax || false;
            const resAirbnbPassThroughTax = resListingInfo?.airbnbPassThroughTax || false;
            const resIsCohostOnAirbnb = resListingInfo?.isCohostOnAirbnb || false;
            const resCohostCommissionCollected = resListingInfo?.cohostCommissionCollected || false;
        ...
        if (priorStatementDuplicateWarnings.length > 0) {
            logger.info(`Found ${priorStatementDuplicateWarnings.length} cross-statement duplicate expenses`, { context: 'StatementsFile', action: 'generateStatement' });
        }
        ...
        const statement = {
            ...
            cohostCommissionCollected: Boolean(listingInfo?.cohostCommissionCollected),
            airbnbPassThroughTax: airbnbPassThroughTax,
            disregardTax: disregardTax,
            cleaningFeePassThrough: cleaningFeePassThrough,
            excludeCleaningFromCommission: Boolean(listingInfo?.excludeCleaningFromCommission),
            ...
            items: [
                ...
                // Expenses and upsells - non-duplicate filtered expenses
                ...nonDuplicateFilteredExpenses.map(exp => {
                    const isUpsell = exp.amount > 0 || (exp.type && exp.type.toLowerCase() === 'upsell') || (exp.category && exp.category.toLowerCase() === 'upsell') || (exp.expenseType === 'extras');
                ...
                // LL Cover expenses (excluding prior statement duplicates)
                ...nonDuplicateLLCover.map(exp => {
                    const isUpsell = exp.amount > 0 || (exp.type && exp.type.toLowerCase() === 'upsell') || (exp.category && exp.category.toLowerCase() === 'upsell') || (exp.expenseType === 'extras');
    ...
    try {
        ...
        if (existingStatement.isCombinedStatement && propertyIds && propertyIds.length > 1) {
            ...
            for (const res of allReservations) {
                const revenue = res.hasDetailedFinance ? res.clientRevenue : (res.grossAmount || 0);
                totalRevenue += revenue;
                const resListing = propertyListingMap[res.propertyId] || {};
                const resPmFee = StatementCalculationService.getEffectivePmFee(resListing, res.createdAt);
        ...
        // Calculate PM commission per-reservation (accounts for PM fee transitions)
        let pmCommission = 0;
        for (const res of periodReservations) {
            const isAirbnb = res.source && res.source.toLowerCase().includes('airbnb');
            if (isAirbnb && isCohostOnAirbnb) continue;
            const revenue = res.hasDetailedFinance ? res.clientRevenue : (res.grossAmount || 0);
            const resPmFee = StatementCalculationService.getEffectivePmFee(listing, res.createdAt);
            pmCommission += revenue * (resPmFee / 100);
        }
        const avgPmPercentage = totalRevenue > 0 ? (pmCommission / totalRevenue) * 100 : (listing.pmFeePercentage ?? 15);
        ...
        let grossPayoutSum = 0;
        for (const res of periodReservations) {
            const resPmPct = StatementCalculationService.getEffectivePmFee(listing, res.createdAt);
            const isAirbnb = res.source && res.source.toLowerCase().includes('airbnb');
            const isCohostAirbnb = isAirbnb && isCohostOnAirbnb;
            const clientRevenue = res.hasDetailedFinance ? res.clientRevenue : (res.grossAmount || 0);
            const commissionBase = StatementCalculationService.getCommissionBase(res, listing, clientRevenue);
        ...
        const updatedStatement = {
            ...
            listingSettingsSnapshot: {
                [parseInt(propertyId)]: {
                    ...
                    cohostCommissionCollected: Boolean(cohostCommissionCollected),
                    ...
                    excludeCleaningFromCommission: Boolean(excludeCleaningFromCommission),
                    guestPaidDamageCoverage: Boolean(listing?.guestPaidDamageCoverage),
                    waiveCommission: Boolean(waiveCommission),
                    waiveCommissionUntil: waiveCommissionUntil || null,
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (modified) {
            ...
            // Calculate PM Commission respecting custom reservation values and PM fee transitions
            // - Custom reservations: use user-entered luxuryLodgingFee
            // - Regular reservations: calculate using effective PM fee per reservation
            // Fetch current listing settings for PM fee transition support
            const editPropertyIds = [...new Set((statement.reservations || []).map(r => r.propertyId).filter(Boolean))];
            const editListingMap = {};
            for (const pid of editPropertyIds) {
                editListingMap[pid] = await ListingService.getListingWithPmFee(parseInt(pid));
            }
            statement.pmCommission = Math.round((statement.reservations || []).reduce((sum, res) => {
                if (res.isCustom && res.luxuryLodgingFee !== undefined) {
                    ...
                    // Regular reservation: calculate using effective PM fee (with transition support)
                    const resListing = editListingMap[res.propertyId];
                    const resPmPct = resListing ? StatementCalculationService.getEffectivePmFee(resListing, res.createdAt) : (parseFloat(statement.pmPercentage) || 15);
                    const revenue = res.hasDetailedFinance ? (parseFloat(res.clientRevenue) || 0) : (parseFloat(res.grossAmount) || 0);
                    return sum + (revenue * (resPmPct / 100));
                }
            }, 0) * 100) / 100;
    ...
    try {
        ...
        if (hasSnapshot) {
            ...
            if (statement.propertyId && listingSettingsMap[statement.propertyId]) {
                ...
                statement.cohostCommissionCollected = Boolean(snap.cohostCommissionCollected);
                ...
                statement.excludeCleaningFromCommission = Boolean(snap.excludeCleaningFromCommission);
            ...
            if (statement.propertyIds && Array.isArray(statement.propertyIds) && statement.propertyIds.length > 0) {
                ...
                dbListings.forEach(listing => {
                    ...
                    listingSettingsMap[listing.id] = {
                        ...
                        cohostCommissionCollected: Boolean(listing.cohostCommissionCollected),
                        ...
                        excludeCleaningFromCommission: Boolean(listing.excludeCleaningFromCommission),
                        guestPaidDamageCoverage: Boolean(listing.guestPaidDamageCoverage),
                ...
                if (currentListing) {
                    ...
                    statement.cohostCommissionCollected = Boolean(currentListing.cohostCommissionCollected);
                    ...
                    statement.excludeCleaningFromCommission = Boolean(currentListing.excludeCleaningFromCommission);
                    ...
                    listingSettingsMap[statement.propertyId] = {
                        ...
                        cohostCommissionCollected: Boolean(currentListing.cohostCommissionCollected),
                        ...
                        excludeCleaningFromCommission: Boolean(currentListing.excludeCleaningFromCommission),
        ...
        statement.cohostCommissionCollected = Boolean(statement.cohostCommissionCollected);
        ...
        statement.reservations?.forEach(reservation => {
            const propSettings = listingSettingsMap[reservation.propertyId] || {
                ...
                cohostCommissionCollected: statement.cohostCommissionCollected,
                ...
                excludeCleaningFromCommission: statement.excludeCleaningFromCommission,
            ...
            if (reservation.isCustom) {
                ...
                grossPayout = -StatementCalculationService.getCohostCommissionDeduction(propSettings.cohostCommissionCollected, luxuryFeeToDeduct) - cleaningFeeForPassThrough;
            } else if (shouldAddTax) {
                grossPayout = clientRevenue - luxuryFeeToDeduct + taxResponsibility - cleaningFeeForPassThrough;
            } else {
                grossPayout = clientRevenue - luxuryFeeToDeduct - cleaningFeeForPassThrough;
            ...
            const batchResults = await Promise.all(batch.map(async (property) => {
                try {
                    ...
                    for (const res of periodReservations) {
                        ...
                        const resPmPct = listing ? StatementCalculationService.getEffectivePmFee(listing, res.createdAt) : 15;
                        const clientRevenue = res.hasDetailedFinance ? res.clientRevenue : (res.grossAmount || 0);
                        const pmFee = clientRevenue * (resPmPct / 100);
                        const taxResponsibility = res.hasDetailedFinance ? res.clientTaxResponsibility : 0;
                        // Reverse-engineer actual cleaning fee from guest-paid amount (only when pass-through enabled)
                        // Formula: actualCleaningFee = (guestPaid / (1 + PM%))
                        const guestPaidCleaningFee = res.cleaningFee ?? listing?.cleaningFee ?? 0;
                        ...
                        let grossPayout;
                        if (isCohostAirbnb) {
                            grossPayout = -StatementCalculationService.getCohostCommissionDeduction(cohostCommissionCollected, pmFee) - cleaningFeeForPassThrough;
                        } else if (shouldAddTax) {
        ...
        for (const owner of owners) {
            ...
            for (const property of ownerProperties) {
                try {
                    ...
                    // Get listing settings from database (property-specific)
                    const listing = await ListingService.getListingWithPmFee(property.id);
                    const cleaningFeePassThrough = listing?.cleaningFeePassThrough || false;
                    const excludeCleaningFromCommission = listing?.excludeCleaningFromCommission || false;
                    ...
                    const statement = {
                        ...
                        ownerPayout: Math.round(ownerPayout * 100) / 100,
                        cleaningFeePassThrough: cleaningFeePassThrough,
                        excludeCleaningFromCommission: excludeCleaningFromCommission,
                        // Snapshot listing settings at generation time
                        waiveCommission: Boolean(listing?.waiveCommission),
                        waiveCommissionUntil: listing?.waiveCommissionUntil || null,
                        disregardTax: Boolean(listing?.disregardTax),
